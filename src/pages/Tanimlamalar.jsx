...
export default function Tanimlamalar({ user }) {
  ...
  const [formData, setFormData] = useState({ code: '', owner_name: '', card_category: '', bank: '', expiry_month: '', expiry_year: '', limit_amount: '', balance: '', current_debt: '', name: '', iban: '', statement_day: '', payment_day: '' });
  const [cardType, setCardType] = useState('');
  const [editingId, setEditingId] = useState(null);
  const [errorModal, setErrorModal] = useState({ show: false, message: '' });
  const [successModal, setSuccessModal] = useState({ show: false, message: '' });
  const [conflictModal, setConflictModal] = useState({ show: false, conflicts: [], pendingData: [] });
  const [showDeleteModal, setShowDeleteModal] = useState({ show: false, collection: null, id: null, item: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeTab === 'kredi-karti') {
        ...
        const ibanNumber = formData.iban.replace(/^TR/i, '').replace(/\s/g, '');
    ...
    setFormData({ code: '', owner_name: '', card_category: '', bank: '', expiry_month: '', expiry_year: '', limit_amount: '', balance: '', current_debt: '', name: '', iban: '', statement_day: '', payment_day: '' });
    setCardType('');
    setEditingId(null);
    loadData();
  };
  ...
  const handleEdit = (item) => {
    ...
    if (activeTab === 'kredi-karti') {
      ...
      setFormData({
        ...
        current_debt: '',
        name: '',
        statement_day: item.statement_day?.toString() || '',
  ...
  const handleDelete = async (collection, id) => {
    ...
    setShowDeleteModal({ show: true, collection, id, item: itemToDelete });
  };

  const confirmDelete = async () => {
    const { collection, id, item } = showDeleteModal;
    ...
    if (user && item) {
      ...
      if (collection === 'creditCards') {
        ...
        logDetails = `Kart: ${item.code} | Banka: ${item.bank || '?'}`;
      } else if (collection === 'bankAccounts') {
        logAction = 'Banka Hesabı Silindi';
        logDetails = `Kod: ${item.code} | İsim: ${item.name}`;
    ...
    setShowDeleteModal({ show: false, collection: null, id: null, item: null });
    loadData();
  };
        ...
        if (activeTab === 'cari') {
          ...
          if (existing && replaceConflicts) {
            await firestore.updateDocument('creditCards', existing.id, {
              ...
              statement_day: itemData.statement_day || null,
      ...
      {showDeleteModal.show && (
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          <div 
            ...
            onClick={() => setShowDeleteModal({ show: false, collection: null, id: null, item: null })}
          ></div>
          <div 
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Silme Onayı</h2>
              <button
                onClick={() => setShowDeleteModal({ show: false, collection: null, id: null, item: null })}
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-700 text-lg">Bu kaydı silmek istediğinize emin misiniz?</p>
              </div>
            </div>
            <div className="flex gap-3 mt-8">
              ...
              <button
                onClick={() => setShowDeleteModal({ show: false, collection: null, id: null, item: null })}
                className="flex-1 px-6 py-3 bg-gray-300 text-gray-700 rounded-xl hover:bg-gray-400 transition-all font-semibold"
              >
                İptal
              </button>
            </div>
      ...
      <div style={{maxWidth: '1000px', margin: '0 auto'}}>
        ...
        <div style={{display: 'grid', gridTemplateColumns: '40% 60%', gap: '24px'}}>
          ...
          {activeTab === 'kredi-karti' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Kesim Tarihi (Gün)</label>
                <input
                  type="number"
                  min="1"
                  max="31"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Ödeme Tarihi (Gün)</label>
                <input
                  type="number"
                  min="1"
                  max="31"
          ...
          <div style={{gridColumn: canEdit ? 'auto' : '1 / -1'}}>
            ...
            <div className="mb-3 flex items-center justify-between" style={{paddingRight: '5px'}}>
              <span className="text-sm text-gray-600 font-medium">{filteredData.length} kayıt</span>
              {totalPages > 1 && (
                <div className="flex items-center gap-2" style={{marginRight: '5px'}}>
                  <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-sm font-semibold transition-colors">
                    ←
                  </button>
                  <span className="text-sm text-gray-600 font-medium">Sayfa {currentPage} / {totalPages}</span>
                  <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-sm font-semibold transition-colors">
                    →
                  </button>
                </div>
              )}
          ...
          <table className="w-full min-w-full">
            ...
            <tbody>
              {activeTab === 'kredi-karti' && paginatedData.map(item => (
              ...
              {activeTab === 'banka-hesabi' && paginatedData.map(item => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
              ...
              {activeTab === 'kategori' && paginatedData.map(item => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  <td className="px-3 py-2 text-sm">{item.code}</td>
                  <td className="px-3 py-2 text-sm">{item.name}</td>
                  {canEdit && (
              ...
              {activeTab === 'cari' && paginatedData.map(item => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  <td className="px-3 py-2 text-sm">{item.code || '-'}</td>
                  <td className="px-3 py-2 text-sm">{item.name}</td>
                  {canEdit && (
