...
import { validateCreditCardPayment, validateCariCreditCardPayment } from '../utils/paymentValidation';
import { createPaymentLog } from '../utils/paymentLogger';
import { updateReminders } from '../utils/reminderUpdater';
...
export default function YeniOdeme({ selectedDate, onClose, preSelectedCard, preSelectedCari, reminderId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentType === 'kredi_karti' && formData.credit_card_id) {
      const warning = await validateCreditCardPayment(formData, cards, amount);
      if (warning) {
        setLimitWarningData(warning);
    ...
    if (paymentType === 'cari' && formData.payment_method === 'kredi_karti' && formData.credit_card_id) {
      const warning = await validateCariCreditCardPayment(formData, cards, amount);
      if (warning) {
        setLimitWarningData(warning);
        setShowLimitWarning(true);
        return;
  ...
  const savePayment = async () => {
    ...
    try {
      ...
      const user = JSON.parse(localStorage.getItem('user'));
      if (user) {
        const logDetails = createPaymentLog(paymentType, formData, cards, accounts, cariList);
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({ 
                        ...
                        description: selectedAccount ? `DBS - ${selectedAccount.name}` : formData.description
                      });
                    }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    required
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({ 
                        ...formData, 
                        bank_account_id: e.target.value,
                        description: selectedAccount ? `Banka Mevduat - ${selectedAccount.name}` : formData.description
                      });
                    }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    required
