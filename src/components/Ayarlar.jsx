...
export default function Ayarlar({ onClose }) {
  ...
  return (
    <div style={{position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000}} onClick={onClose}>
      <div style={{background: 'white', borderRadius: '16px', padding: '24px', width: '700px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)'}} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{display: 'flex', gap: '8px', marginBottom: '24px', borderBottom: '2px solid #e5e7eb'}}>
          <button
            ...
            style={{padding: '12px 24px', background: 'transparent', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '14px', color: activeTab === 'genel' ? '#2563eb' : '#6b7280', borderBottom: activeTab === 'genel' ? '2px solid #2563eb' : 'none', marginBottom: '-2px'}}
          >
            Genel
          </button>
          <button
            onClick={() => setActiveTab('senkronizasyon')}
            style={{padding: '12px 24px', background: 'transparent', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '14px', color: activeTab === 'senkronizasyon' ? '#2563eb' : '#6b7280', borderBottom: activeTab === 'senkronizasyon' ? '2px solid #2563eb' : 'none', marginBottom: '-2px'}}
          >
            Senkronizasyon
          </button>
        </div>
        ...
        {activeTab === 'senkronizasyon' && (
          <>
            {syncMessage && (
              <div style={{padding: '12px', borderRadius: '8px', marginBottom: '16px', textAlign: 'center', fontWeight: 600, background: syncMessage.includes('✓') ? '#d1fae5' : syncMessage.includes('✗') ? '#fee2e2' : '#dbeafe', color: syncMessage.includes('✓') ? '#059669' : syncMessage.includes('✗') ? '#dc2626' : '#1e40af'}}>
                {syncMessage}
              </div>
            )}

            <div style={{display: 'flex', flexDirection: 'column', gap: '12px'}}>
              <div style={{padding: '16px', background: '#f0f9ff', border: '2px solid #bfdbfe', borderRadius: '12px', marginBottom: '12px'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px'}}>
                  ...
                  <h4 style={{fontSize: '14px', fontWeight: 600, color: '#1e40af'}}>Otomatik Senkronizasyon Aktif</h4>
                </div>
                <p style={{fontSize: '13px', color: '#1e40af', lineHeight: '1.5'}}>
                  Verileriniz Firestore ile otomatik senkronize ediliyor. Her değişiklik anında kaydedilir.
                </p>
              </div>
              ...
              <button
                onClick={async () => {
                  if (syncing) return;
                  setSyncing(true);
                  setSyncMessage('Dosya kaydediliyor...');
                  const { default: transferManager } = await import('../utils/transferManager');
                  const result = await transferManager.exportData();
                  setSyncMessage(result.success ? `✓ Kaydedildi: ${result.path || 'Downloads'}` : `✗ Hata: ${result.error}`);
                  setSyncing(false);
                  setTimeout(() => setSyncMessage(''), 3000);
                }}
                style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', background: syncing ? '#9ca3af' : 'linear-gradient(to right, #3b82f6, #2563eb)', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', cursor: syncing ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '14px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)'}}
              >
                <svg style={{width: '24px', height: '24px'}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <label style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', background: syncing ? '#9ca3af' : 'linear-gradient(to right, #10b981, #059669)', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', cursor: syncing ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '14px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)'}}>
                <svg style={{width: '24px', height: '24px'}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                <span>{syncing ? 'Yükleniyor...' : 'Dış Dosya Yükle'}</span>
                <input
                  type="file"
                  accept=".finans"
                  onChange={async (e) => {
                    ...
                    if (!file) return;
                    setSyncing(true);
                    setSyncMessage('Dosya yükleniyor...');
                    const { default: transferManager } = await import('../utils/transferManager');
                    const result = await transferManager.importData(file, 'merge');
                    setSyncMessage(result.success ? '✓ Veri yüklendi' : `✗ Hata: ${result.error}`);
                    setSyncing(false);
                    if (result.success) {
