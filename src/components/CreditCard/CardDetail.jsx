...
export default function CardDetail({ card, onBack, onReload, canEdit = true }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('cardDetailUpdated', handleUpdate);
    return () => window.removeEventListener('cardDetailUpdated', handleUpdate);
  }, []);

  const loadPayments = async () => {
    const allPayments = await firestore.getPayments();
    const filtered = allPayments
      .filter(p => p.credit_card_id === card.id && (p.is_completed === true || p.payment_method === 'devir'))
